...
/* Pin connection*/
#define CFG_NO_RST
#define LCD_RESET_GPIO_Port //ST7789_RST_GPIO_Port
#define LCD_RESET_Pin  //ST7789_RST_Pin
#define LCD_DCX_GPIO_Port  GPIOE
#define LCD_DCX_Pin   GPIO_PIN_15
...
void lv_port_disp_init(void)
{
    ...
    lv_display_set_offset(lcd_disp, 0, 20);
...
static int32_t lcd_io_init(void)
{
    ...
    HAL_SPI_RegisterCallback(&hspi4, HAL_SPI_TX_COMPLETE_CB_ID, lcd_color_transfer_ready_cb);
...
{
    LV_UNUSED(disp);
    // printf("[lv_port_disp.c]lcd_send_cmd begin\n\r");
    // printf("[lv_port_disp.c]lcd_send_cmd cmd:0x%x\n\r", *cmd);
   ...
   // HAL_GPIO_WritePin(LCD_CS_GPIO_Port, LCD_CS_Pin, GPIO_PIN_RESET);
    ...
    if(HAL_SPI_Transmit(&hspi4, cmd, cmd_size, BUS_SPI1_POLL_TIMEOUT) == HAL_OK) {
        ...
        printf("[lv_port_disp.c]lcd_send_cmd error in HAL_SPI_Transmit\n\r");
    }
    // printf("[lv_port_disp.c]lcd_send_cmd end\n\r");
}
...
static void lcd_send_color(lv_display_t * disp, const uint8_t * cmd, size_t cmd_size, uint8_t * param,
                           size_t param_size)
{
    // printf("[lv_port_disp.c]lcd_send_color begin\n\r");
    // printf("[lv_port_disp.c]lcd_send_color cmd:0x%x\n\r", *cmd);
    LV_UNUSED(disp);
    while(lcd_bus_busy);    /* wait until previous transfer is finished */
    /* Set the SPI in 8-bit mode */
    //hspi4.Init.DataSize = SPI_DATASIZE_8BIT;
    ...
    if(HAL_SPI_Transmit(&hspi4, cmd, cmd_size, BUS_SPI1_POLL_TIMEOUT) == HAL_OK) {
        ...
        //HAL_SPI_Init(&hspi4);
        lcd_bus_busy = 1;
        //HAL_SPI_Transmit_DMA(&hspi4, param, (uint16_t)param_size / 2);
        HAL_SPI_Transmit_DMA(&hspi4, param, param_size);
        /* NOTE: CS will be reset in the transfer ready callback */
    } else {
        printf("[lv_port_disp.c]lcd_send_color error in HAL_SPI_Transmit\n\r");
    }

    //printf("[lv_port_disp.c]lcd_send_color end\n\r");
}
